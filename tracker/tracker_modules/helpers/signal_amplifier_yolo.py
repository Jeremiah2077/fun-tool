"""
Signal Amplifier Helper Module - YOLO ROI Version

Provides enhanced signal amplification and mastering techniques for YOLO ROI tracker.
This version USES the pre-calculated positions from adaptive scaling as a base.

Key differences from oscillation version:
- Uses raw_primary/raw_secondary as base (NOT fixed 50)
- Applies flow enhancement ON TOP of adaptive scaling
- Designed for yolo_roi tracker's workflow
"""
...
class SignalAmplifierYolo:
    """
    Signal amplification and mastering for YOLO ROI tracker position data.

    This class enhances positions that were already calculated by adaptive scaling.
    Unlike the oscillation version, this DOES NOT start from neutral position 50.
    """
    ...
    def enhance_signal(self,
        ...
        """
        Apply enhanced signal amplification to YOLO ROI tracker positions.

        YOLO VERSION: Now works the same as oscillation version!
        - Starts from neutral position 50
        - Uses dy_flow/dx_flow to calculate position
        - Applies live_amp for better range

        The difference from adaptive scaling is that this uses SignalAmplifier's
        enhanced flow scaling and live dynamic amplification.

        Args:
            raw_primary: Not used (kept for compatibility)
            raw_secondary: Not used (kept for compatibility)
        ...
        try:
            ...
            # Start from neutral position 50 (same as oscillation)
