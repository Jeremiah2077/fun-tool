...
class OscillationExperimental2Tracker(BaseTracker):
    ...
    def _initialize_optical_flow(self):
        ...
        self.use_farneback_fallback = False
        try:
            ...
            try:
                ...
                # Final fallback: use Farneback optical flow
                self.logger.warning(f"DIS optical flow not available: {e}. Using Farneback fallback.")
                self.use_farneback_fallback = True
                self.flow_dense_osc = True  # Set to True to indicate flow is available
    ...
    def _process_oscillation_experimental_2(self, frame: np.ndarray, frame_time_ms: int, 
        ...
        # Use appropriate optical flow method
        if hasattr(self, 'use_farneback_fallback') and self.use_farneback_fallback:
            # Farneback optical flow fallback
            flow = cv2.calcOpticalFlowFarneback(
                prev_gray_cont, current_gray_cont, None,
                pyr_scale=0.5, levels=3, winsize=15,
                iterations=3, poly_n=5, poly_sigma=1.2, flags=0
            )
        else:
            # DIS optical flow (original method)
