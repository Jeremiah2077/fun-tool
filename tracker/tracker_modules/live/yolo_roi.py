...
try:
    ...
    from ..helpers.signal_amplifier_yolo import SignalAmplifierYolo
except ImportError:
    from tracker.tracker_modules.core.base_tracker import BaseTracker, TrackerMetadata, TrackerResult
    from tracker.tracker_modules.helpers.signal_amplifier_yolo import SignalAmplifierYolo
...
class YoloRoiTracker(BaseTracker):
    ...
    def __init__(self):
        ...
        # Enhanced signal mastering using YOLO-specific helper module
        # Now works same as oscillation: starts from 50, uses flow + live_amp
        self.signal_amplifier = SignalAmplifierYolo(
            history_size=120,  # 4 seconds @ 30fps
            enable_live_amp=True,  # Enable live dynamic amplification for better range
            smoothing_alpha=0.7,
            logger=self.logger
        )
    ...
    def process_main_roi_content(self, processed_frame_draw_target: np.ndarray, current_roi_patch_gray: np.ndarray, prev_roi_patch_gray: Optional[np.ndarray], prev_sparse_features: Optional[np.ndarray]) -> Tuple[int, int, float, float, Optional[np.ndarray]]:
        ...
        # Apply enhanced signal mastering using SignalAmplifier
        # This starts from 50 and uses dy_smooth/dx_smooth with live_amp
